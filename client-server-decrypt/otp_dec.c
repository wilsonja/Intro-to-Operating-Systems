 ...
 *  in decryption process. Communicates with
 *  decryption server.
 * Set up borrowed heavily from CS344 lecture.
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(buffer, "def") != 0) {
    perror("ERROR: handshake failed, cannot connect to otp_enc_d\n");
    exit(1);
  };

  // send message for decryption and wait for confirmation from server
  sendFile(argv[1], sockFD);
  memset(messageBuffer, 0, sizeof(messageBuffer));
  confirm = read(sockFD, messageBuffer, sizeof(messageBuffer));

  // send key to server and wait for receipt confirmation from server
  sendFile(argv[2], sockFD);
  memset(keyBuffer, 0, sizeof(keyBuffer));
  confirm = read(sockFD, keyBuffer, sizeof(keyBuffer));

  // receive and print decrypted message
  memset(buffer, 0, sizeof(buffer));
...
/* REFERENCES
 * for send/receive over sockets:
 * http://stackoverflow.com/questions/2014033/send-and-receive-a-file-in-
 * socket-programming-in-linux-with-c-c-g-cc-g
 *
 * http;//stackoverflow.com/questions/19794764/linux-socket-how-to-make-
 * and-send-wait-for-recv
 *
 * for ciphers:
 * https://www.khanacademy.org/computing/computer-science/cryptography/
 * ciphers/a/shift-cipher */
